...
for i=176:200
    gradAmp1(1,i) =  -gprephase; %X gradients in Tesla per meter
    gradAmp1(3,i) =  2*gAmprefoc; %Z gradients in Tesla per meter
end
...
%Regarding the reconstructed image, what kind of contrast is this? Why is the T1 weighting so weak?
% this should be a T1w image because the mxy is spoiled.  It is weak
% because there isn't an iversion pulse which is usually used. 
...
for k=1:xSteps
    ...
    for j=1:ySteps
        for i=1:zSteps     
            dB0 = pos(:,k,j)'*gradAmp(:,1); %dB0 = pos(:,k,j)'*gradAmp(:,1);
            [mT,mZ] = bloch(1.2, dB0,rfPulse(1), T1(k,j),T2(k,j), 0, -1);   % makes mxy 0
            for t=2:nTimeSteps %t starts at 2
                dB0 = pos(:,k,j)'*gradAmp(:,t); %dB0 = pos(:,k,j)'*gradAmp
                [mT,mZ] =  bloch(dt, dB0,rfPulse(t), T1(k,j),T2(k,j), mT, mZ); 
                if(adc(1,t)>0)
...
% Regarding the reconstructed image, is this TI optimal?

% it is not much better, but it would be best to use an inversion pulse and
% would not be possible in a real experiment.